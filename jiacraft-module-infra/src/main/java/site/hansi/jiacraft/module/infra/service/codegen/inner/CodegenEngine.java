package site.hansi.jiacraft.module.infra.service.codegen.inner;
...
import site.hansi.jiacraft.framework.apilog.core.enums.OperateTypeEnum;
...
import site.hansi.jiacraft.framework.excel.core.annotations.DictFormat;
import site.hansi.jiacraft.framework.excel.core.convert.DictConvert;
...
import site.hansi.jiacraft.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 jiacraft-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 jiacraft-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("site.hansi.jiacraft.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/jiacraft-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/jiacraft-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "jiacraft-module-${table.moduleName}/" + // 顶级模块
                "jiacraft-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "jiacraft-module-${table.moduleName}/" + // 顶级模块
                "jiacraft-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "jiacraft-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "jiacraft-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "jiacraft-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
